...
class EmbeddingVectorizer(BaseEstimator, TransformerMixin, _VectorizerMixin):
    ...
    def _check_pooling_function(self, function: Callable) -> bool:
        ...
        if pooled.shape != (300,):
            raise ValueError(
                f"Your pooling function returned the wrong shape. Expected (300,), got {pooled.shape}"
            )
    ...
    def fit(self, X: ArrayLike, y: Optional[List[Label]] = None) -> EmbeddingVectorizer:
        try:
            ...
            raise ValueError(
                f"Your pooling function was not in the set of pooling functions. Got '{self.pooling_function}', expected one of {set(POOLING.keys())} "
            )
